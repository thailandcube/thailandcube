...
function numToFormatted(time: number | undefined | null, isResult: boolean = false): string {
  if (time === 0)
    return '';
...
function formattedToNum(time: string | null | undefined): number {
  if (!time)
    return 0;
...
function dateToRange(startDate: Date | string, endDate: Date | string): string {
